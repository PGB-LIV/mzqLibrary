...
//            This part will be improved later when we add CV terms to mzq describing which protein is group leader and which are same or sub-sets etc
            ...
            for (String quantityName : MzqLib.data.getQuantitationNames()) {
                if (MzqLib.data.control.isRequired(MzqData.PROTEIN_GROUP, MzqData.ASSAY, quantityName) 
                        || MzqLib.data.control.isRequired(MzqData.PROTEIN_GROUP, MzqData.SV, quantityName)){
            ...
            if (proteinLevel == MzqData.PROTEIN_GROUP){
                proCvParam = determineQuantitationUnit(MzqData.PROTEIN_GROUP, names);
            }else{
            ...
            if (proCvParam == null && pepCvParam == null) {
                tabDesc = new MZTabDescription(MZTabDescription.Mode.Summary, MZTabDescription.Type.Quantification);
                isComplete = false;
            } else {
            ...
            for (int i = 0; i < cvs.size(); i++) {
                ...
                if (cv.getId().equalsIgnoreCase("PSI-MS")||cv.getId().equalsIgnoreCase("MS")) msCVstr = cv.getId();
                if (cv.getId().equalsIgnoreCase("PRIDE")) hasPRIDEcv = true;
            ...
            if (msCVstr.length()==0){
                ...
                tabCv.setLabel("MS");
                tabCv.setFullName("Proteomics Standards Initiative Mass Spectrometry Vocabularies");
                msCVstr = "MS";
                tabCv.setUrl("http://psidev.cvs.sourceforge.net/viewvc/*checkout*/psidev/psi/psi-ms/mzML/controlledVocabulary/psi-ms.obo");
                mtd.addCV(tabCv);
            }
            ...
            mtd.addProteinSearchEngineScoreParam(1, new CVParam(msCVstr, "MS:1001153", "search engine specific score", null));
            mtd.addPeptideSearchEngineScoreParam(1, new CVParam(msCVstr, "MS:1001153", "search engine specific score", null));
            ...
            HashMap<CvParam, Integer> modCount = new HashMap<>();
            ...
            String methodAccession = mtd.getQuantificationMethod().getAccession();
            //assays
            final ArrayList<Assay> assays = MzqLib.data.getAssays();
            HashMap<String, uk.ac.ebi.pride.jmztab.model.Assay> assayMap = new HashMap<>();
            ArrayList<uk.ac.ebi.pride.jmztab.model.Assay> tabAssays = new ArrayList<>();
//            boolean isSILAC = false;
            CVParam ms1Light = new CVParam("PRIDE", "PRIDE:0000326", "SILAC light", null);
            CVParam ms1Heavy = new CVParam("PRIDE", "PRIDE:0000325", "SILAC heavy", null);
//            CVParam ms1Medium;
...
//                  tabCv.setFullName();
                    ...
                    tabCv.setUrl("https://code.google.com/p/ebi-pride/source/browse/trunk/pride-core/schema/pride_cv.obo");
                    mtd.addCV(tabCv);
                }
            }
            for (int i = 0; i < assays.size(); i++) {
                ...
                if(methodAccession.equals("MS:1001834") || methodAccession.equals("MS:1001836")){//label free 
                    tabAssay.setQuantificationReagent(new uk.ac.ebi.pride.jmztab.model.CVParam(msCVstr,"MS:1002038","unlabeled sample",null));
...
//                    for (ModParam mod : assay.getLabel().getModification()) {
                    ...
                    if (acc.equalsIgnoreCase("MS:1002038")){
                        ...
                        for (ModParam mod1 : assay.getLabel().getModification()) {
                            ...
                            AssayQuantificationMod assayQuantMod = new AssayQuantificationMod(tabAssay, m);
                    ...
                    for (ModParam mod : assay.getLabel().getModification()) {
                        ...
                        AssayQuantificationMod assayQuantMod = new AssayQuantificationMod(tabAssay, 1);
                        assayQuantMod.setParam(assayMod);
                        tabAssay.addQuantificationMod(assayQuantMod);
                    }
                }
...
//            }
            ...
            proFactory.addBestSearchEngineScoreOptionalColumn(ProteinColumn.BEST_SEARCH_ENGINE_SCORE, 1);
            pepFactory.addBestSearchEngineScoreOptionalColumn(PeptideColumn.BEST_SEARCH_ENGINE_SCORE, 1);
            for (int i = 0; i < mtd.getMsRunMap().size(); i++) {
                MsRun msrun = mtd.getMsRunMap().get(i+1);
                proFactory.addSearchEngineScoreOptionalColumn(ProteinColumn.SEARCH_ENGINE_SCORE, 1, msrun);
                pepFactory.addSearchEngineScoreOptionalColumn(PeptideColumn.SEARCH_ENGINE_SCORE, 1, msrun);
            }
            ...
            if (proCvParam != null) {//means no assay quant layer for protein, no need to add more
                ...
                for (String quantName : names) {
                    ...
                    if (MzqLib.data.control.isRequired(proteinLevel, MzqData.ASSAY, quantName)) {
                        for (uk.ac.ebi.pride.jmztab.model.Assay tabAssay : tabAssays) {
                            proFactory.addOptionalColumn(tabAssay, quantName, String.class);
                        }
                    }
                    if (MzqLib.data.control.isRequired(proteinLevel, MzqData.SV, quantName)) {
                        for (uk.ac.ebi.pride.jmztab.model.StudyVariable sv : tabSvs) {
                            proFactory.addOptionalColumn(sv, quantName, String.class);
                        }
                    }
                }
                if (MzqLib.data.control.isRequired(proteinLevel, MzqData.RATIO, MzqData.RATIO_STRING)) {
                    for (String ratioID : ratioIDs) {
                        proFactory.addOptionalColumn(ratioID, String.class);
                    }
                }
                final HashSet<String> elements = MzqLib.data.control.getElements(proteinLevel, MzqData.GLOBAL);
                if (!elements.isEmpty()){
            ...
            if (pepCvParam != null) {
                ...
                final HashSet<String> elements = MzqLib.data.control.getElements(MzqData.PEPTIDE, MzqData.GLOBAL);
                if (!elements.isEmpty()) {
                    for (String str : elements) {
            ...
            for (QuantitationLevel entity : proEntities) {
                ...
                if (!protein.getAccession().equals(MzqData.ARTIFICIAL)) {
                    ...
                    if (proCvParam != null) {
                        if (MzqLib.data.control.isRequired(proteinLevel, MzqData.ASSAY, proCvParam.getName())) {
                            for (int i = 0; i < assays.size(); i++) {
                                String assay = assays.get(i).getId();
                                final Double value = entity.getQuantity(proCvParam.getName(), assay);
                                if (value != null) {
                        ...
                        if (MzqLib.data.control.isRequired(proteinLevel, MzqData.SV, proCvParam.getName())) {
                            for (int i = 0; i < svs.size(); i++) {
                                StudyVariable sv = svs.get(i);
                                final Double value = entity.getStudyVariableQuantity(proCvParam.getName(), sv.getId());
                                if (value != null) {
                        ...
                        for (String quantName : names) {
                            ...
                            if (MzqLib.data.control.isRequired(proteinLevel, MzqData.ASSAY, quantName)) {
                                for (int i = 0; i < assays.size(); i++) {
                                    ...
                                    final Double value = entity.getQuantity(quantName, assayID);
                                    if (value != null) {
                            ...
                            if (MzqLib.data.control.isRequired(proteinLevel, MzqData.SV, quantName)) {
                                for (int i = 0; i < svs.size(); i++) {
                                    StudyVariable sv = svs.get(i);
                                    final Double value = entity.getStudyVariableQuantity(quantName, sv.getId());
                                    if (value != null) {
...
//                                }
                        ...
                        for (FeatureData feature : peptide.getFeaturesWithCharge(charge)) {
                            ...
                            if (rtStr != null && !rtStr.equalsIgnoreCase("null")) {
                                ...
                                if (maxRt < rt) maxRt = rt;
                                if (minRt > rt) minRt = rt;
                                rts.add(rt);
                            }
                            String specRef = feature.getFeature().getSpectrumRefs();
                            if (specRef != null){
                                int msrun_id = msruns.get(feature.getRawFilesGroupRef());
                        ...
                        if (!spectrum_refs.isEmpty()) tabPep.setSpectraRef(spectrum_refs);
                        ...
                        if (pepCvParam != null) {
                            if (MzqLib.data.control.isRequired(MzqData.PEPTIDE, MzqData.ASSAY, pepCvParam.getName())) {
                                for (int i = 0; i < assays.size(); i++) {
                                    ...
                                    if (value != null) {
                                        tabPep.setAbundanceColumnValue(assayMap.get(assay), value);
                                    }
                                }
                            }
                            if (MzqLib.data.control.isRequired(MzqData.PEPTIDE, MzqData.SV, pepCvParam.getName())) {
                                for (int i = 0; i < svs.size(); i++) {
                                    ...
                                    if (value != null) {
                                        tabPep.setAbundanceColumnValue(tabSvs.get(i), value);
                                    }
                                }
                            }
                            String pepCvName = pepCvParam.getName();
...
//                    }
//                }else{
                    mtd.addFixedModParam(1, new CVParam(msCVstr, "MS:1002453", "No fixed modifications searched", null));
                    mtd.addVariableModParam(1, new CVParam(msCVstr, "MS:1002454", "No variable modifications searched", null));
//                }
            }else{
                for (CvParam qParam : modCount.keySet()) {
                    int count = modCount.get(qParam);
                    if (count < pepCount) {
                        mtd.addVariableModParam(variableModCount, Utils.convertMztabParam(qParam));
                        variableModCount++;
                    } else {
                        mtd.addFixedModParam(fixModCount, Utils.convertMztabParam(qParam));
                        fixModCount++;
                    }
                }
            }
...
//            out.append(mztab.toMzTab());
    ...
    private CvParam determineQuantitationUnit(int level, ArrayList<String> names) {
        ...
        final HashSet<String> global = MzqLib.data.control.getElements(level, MzqData.GLOBAL);
        if (!global.isEmpty()){
