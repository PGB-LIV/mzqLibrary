...
public class XlsConverter extends GenericConverter {
    ...
    public final void convert() {
        if (this.getOutfile().length() == 0) {
            this.setOutfile(getBaseFilename() + ".xls");
        }

        try {
            WritableWorkbook wb = Workbook.createWorkbook(new File(this.getOutfile()));
    ...
    private int outputAssayAndSV(final int level, final WritableSheet sheet, final List<QuantitationLevel> objects)
    ...
    private void outputGlobal(final int level, final WritableSheet sheet, final List<QuantitationLevel> objects,
                              int rowCount)
            throws NumberFormatException, WriteException {
        ...
        for (String columnID : MzqLib.DATA.getControl().getElements(level, MzqData.GLOBAL)) {
    ...
    private void outputMetadata(final WritableSheet metaSheet) throws NumberFormatException, WriteException {
        int rowCount = 0;
    ...
    private int outputRatio(final int level, final WritableSheet sheet, final List<QuantitationLevel> objects,
                            int rowCount)
            throws NumberFormatException, WriteException {
    ...
    private void printQuantitationLevel(final int level, final WritableSheet sheet, final int rowCount,
    ...
    private void printValue(final Double value, final WritableSheet sheet, final int colCount, final int rowCount)
