...
public class LoadMzQuantMLDataTask extends Task<MzQuantMLData> {
    ...
    protected MzQuantMLData call() throws Exception {
        ...
        cvParams.stream()
                .parallel()
                .filter((cp) -> !cp.getName().contains("level"))
                .forEach(
                    (cp) -> {
...
//                      }
        ...
        if (protGrpList != null) {
            ...
            for (QuantLayer assQL : protGrpAssQLs) {
                ...
                for (Row row : rows) {
                    ...
                    ProteinGroup     protGrp  = mzqUm.unmarshal(ProteinGroup.class, row.getObjectRef());    // get ProteinGrouop object from objectRef
                    ...
                    for (ProteinRef protRef : protRefs) {
                        Protein protein = mzqUm.unmarshal(Protein.class, protRef.getProteinRef());

                        proteinGroupAccessions.append(protein.getAccession()).append(" ;");
                    }

                    mzqDMRow.setObjectValue(
                        new SimpleStringProperty(
                            proteinGroupAccessions.substring(0, proteinGroupAccessions.length() - 2)));
        ...
        if (protList != null) {
            ...
            for (QuantLayer assQL : protAssQLs) {
                ...
                for (Row row : rows) {
                    MzqDataMatrixRow mzqDMRow = new MzqDataMatrixRow();
                    Protein          protein  = mzqUm.unmarshal(Protein.class, row.getObjectRef());
        ...
        Iterator<PeptideConsensusList> peptideListIter =
        ...
        if (peptideListIter != null) {
            ...
            while (peptideListIter.hasNext()) {
                ...
                for (QuantLayer assQL : pepAssQLs) {
                    ...
                    for (Row row : rows) {
                        MzqDataMatrixRow mzqDMRow      = new MzqDataMatrixRow();
                        PeptideConsensus peptide       = mzqUm.unmarshal(PeptideConsensus.class, row.getObjectRef());
                        ...
                        if (!peptide.getModification().isEmpty()) {
                            ...
                            mzqDMRow.setObjectValue(
                                new SimpleStringProperty(peptideString.substring(0, peptideString.length() - 2)));
                        } else {
        ...
        Iterator<FeatureList>                featureListIter     =
            mzqUm.unmarshalCollectionFromXpath(MzQuantMLElement.FeatureList);
